...
class SimuladorOficial:
    ...
    def _escribir_pose(self):
        ...
        if not e.get("de_pie", True):
            for idx, valor in self.robot.pose_sentado.items():
                if idx < len(art):
                    art[idx] = valor
        elif e["accion"] in self.robot.gestos:
            self._aplicar_gesto(art, q, e)
        elif e["moviendose"]:
            f = e["fase"]
    ...
    def _aplicar_gesto(self, art, q, estado) -> None:
        """Dibuja el gesto en curso, interpolando desde la pose de pie.

        Antes esto era una rama con los nombres puestos a mano que comparaba
        contra "saludando", pero por el socket la accion llega como "saludo":
        el saludo del alumno cambiaba el estado y el robot no movia la mano.
        Ahora el nombre se busca en la tabla del robot, que es la misma que
        deja escrita `mundo.accion`, asi que no hay dos listas que sincronizar.
        """
        from .robots import envolvente

        gesto = self.robot.gestos[estado["accion"]]
        progreso = float(estado.get("progreso_gesto", 0.0))
        f = envolvente(gesto, progreso)

        for idx, objetivo in gesto.articulaciones.items():
            if idx < len(art):
                # Desde donde esta parado hasta la pose del gesto. Interpolar
                # (en vez de escribir el objetivo) es lo que lo hace un
                # movimiento y no un cambio de imagen.
                base = self.robot.pose_de_pie.get(idx, 0.0)
    ...
    def _correr_con_ventana(self):
        ...
        # Teclado: solo tiene sentido con la ventana abierta, porque las teclas
        # las reparte el visor. Ver teclado.py.
        mando = None
        if self.teclado:
            from .teclado import Teleoperador

            mando = Teleoperador(self.mundo, self.robot, verboso=self.verboso)
