...
class Mundo:
    ...
    def gesto(self, nombre: str, duracion: float = 2.0) -> None:
        with self._lock:
            ...
            self._vence_en = 0.0
            self.accion = nombre
            # La duracion se guarda para poder decir CUANTO falta, no solo si
            # el gesto sigue vivo: el simulador la necesita para animarlo.
            self._gesto_duracion = max(1e-3, float(duracion))
            self._gesto_hasta = time.monotonic() + self._gesto_duracion
    ...
    def _progreso_gesto(self) -> float:
        """Cuanto avanzo el gesto en curso, de 0 a 1. Se llama con el lock.

        El simulador lo usa para animarlo: sin esto solo se sabe si el gesto
        sigue vivo, y la unica forma de dibujarlo era escribir la pose entera
        de golpe.
        """
        if not self._gesto_hasta or self._gesto_duracion <= 0.0:
            return 0.0
        restante = self._gesto_hasta - time.monotonic()
        return min(1.0, max(0.0, 1.0 - restante / self._gesto_duracion))
    ...
    def leer(self) -> dict:
        with self._lock:
            return {
                ...
                "fase": self.fase,
                "progreso_gesto": self._progreso_gesto(),
            }
